...
source ${XDG_CONFIG_HOME}/wofi-pubs/config

PDFVIEWER=${pdfviewer:-/usr/bin/zathura}
WOFI=${wofi:-/usr/bin/wofi}
PUBS=${pubs:-/usr/bin/pubs}
CONFIGS_DIR=${configs_dir:-$HOME/.config/pubs}
DEFAULT_LIB=${default_lib:-$HOME/.config/pubs/main_lib.conf}
CACHE_AUTH=${cache_auth:-$HOME/.local/tmp/pubs_wofi_auth}
CACHE_LIBS=${cache_libs:-$HOME/.local/tmp/pubs_wofi_libs}
TERMINAL_EDIT=${terminal_edit:-termite}

BIB_PARSE=$HOME/.local/bin/parse-bib-file
PUBS_TO_DPT=$HOME/.local/bin/pubs_to_dptrp1
PUBS_UTILS=$HOME/.local/bin/pubs-utils
UPDATE_METADATA=$HOME/.local/bin/update_pdf_meta
MAKE_WRITABLE=$HOME/.local/bin/make_writable


# List all the publications of the given library
# $1 : library
...
function menu_more_actions() {
    ...
    case $selected in
      "add document") add_doc ${bibkey} ${lib_conf};;
           "add tag") add_tag ${bibkey} ${lib_conf};;
       "send e-mail") send_mail ${bibkey} ${lib_conf};;
              "back") menu_ref ${bibkey} ${lib_conf};;
    esac
}


# Show publications from the same author(s) as a given paper
...
function add_doc() {
    ...
    # Update metadata
    # $MAKE_WRITABLE
    # Get doc directory of library
    doc_lib = $(cat $1 | awk '/^docsdir/ {print $3}')
    ${UPDATE_METADATA} $2 ${1}
}
